            ...
            <Table my="8" borderWidth="1px" fontSize="sm">
                <Thead bg={useColorModeValue("gray.50", "gray.800")}>
                  <Tr>
                    ...
                    <Th whiteSpace="nowrap" scope="col"> Processor </Th>
                ...
                <Tbody>
                  {data.map((row, index) => (
                    <Tr key={index}>
                      <Td> {index+1} </Td>
                      <Td> {row.po_number} </Td>
                      <Td> {row.get_processor.processor_name} </Td>
                      <Td> {moment(row.created_at).format('DD / MMM / YYYY H:mm:s')} </Td>
                      <Td>
                        <Center>
                          <Link href={`/exportir/po/preview/${encodeURIComponent(row.id)}`} passHref={true}>
                            <Button variant="outline" size="sm"  colorScheme="yellow">
                              Preview
                            </Button>
                          </Link>
                          &nbsp;&nbsp;
                          <Link href={`/exportir/po/edit/${encodeURIComponent(row.id)}`} passHref={true}>
